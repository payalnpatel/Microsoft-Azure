...
- knowledge mining - the term used to describe solutions that involve extracting information from large volumes of often unstructured data.
- Azure AI Search - knowledge mining solution - a cloud search service that has tools for building user-managed indexes. the indexes can be used for internal use only, or to enable searchable content on public-facing internet assets.
- Azure AI Search can utilize the built-in capabilities of Azure AI servcies such as image processing, content extraction, and natural language processing to perform knowledge mining of documents.
...
- Azure AI Search - platform as a service solution. programmable search engine built on Apache lucene, an open-source software library.
- A typical search solution starts with a data source that contains the data artifacts you want to search. Azure AI Search supports JSON. then an indexer can be used to automate data ingestion, this indexer connects to a data source, serializes the data, and passes to the search engine for indexing. indexers also support AI Enrichment.
- AI Enrichment refers to embedded image and NLP in a pipeline that extracts text and info from content that can't otherwise be indexed for full text search.
...
- for each field in the document, the index stores its name, the data type, and supported behaviors for the field.
- in order to index documents in Azure Storage, they need to be exported from their original file type to JSON.
- knowledge store - persistent storage of enriched content. purpose of a knowledge store is to store the data generated from AI enrichment in a container. a knowledge store can contain one or more of three types of projection of the extracted data - table projections, object projections (JSON documents), file projections

### Fundamentals of Generative AI
- Generative AI describes a category of capabilities within AI that create original content. people typically interact with generative AI that has been built into chat applications. one popular example is Microsoft Coplot - a chatbot companion to browse the web more effectively.
...
- Image generation, code generation - some output examples of gen AI apps
- Generative AI applications are powered by language models, which are a specialized type of machine learning model that you can use to perform NLP tasks, including:
  - determining sentiment or otherwise classifying natural language text
  - summarizing text
  - comparing multiple text sources for semantic similarity
  - generating new natural language
- ML models for NLP have evolved over the years. today's cutting edge LLMs are based on the transformer architecture, which builds on and extends some techniques that have been proven successful in modeling vocabularies to support NLP tasks - and in particular in generating language.
...
- Transformer model architecture consists of 2 components, or blocks:
...
- the encoder block processes token sequences using a technique called attention to determine relationships between tokens
- output from an encoder is a collection of vectors -- these vectors are referred to as embeddings -- each element of the vector/embedding represents a semantic attribute
...
- specific implementations of transformer architecture will vary -- for exmaple Generative Pretrained Transformer (GPT) model developed by OpenAI uses only the decoder block.
- the first step in training a transformer model is to decompose the training text into tokens.
- to create a vocabulary that encapsulates semantic relationships between the tokens, we define contextual vectors, known as embeddings. vectors are multi-valued numeric representations of information, in which each numeric element represents a particular attribute of the information. for language tokens, each element of a token's vector represents some semantic attribute of the token.
- vectors represent lines in multi-dimensional space, describing direction and distance along multiple axes.
- the encoder and decoder blocks in a transformer model include multiple layers that form the neural network for the model. attention layers are used in both blocks - attention is a technique used to examine a sequence of text tokens and try to quantify the strength of the relationships between them. self-attention involves considering how other tokens around one particular token influence that token's meaning.
- 
- -
